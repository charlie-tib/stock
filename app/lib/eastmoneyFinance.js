...
function normalizeFinanceRow(row) {
  return {
    period: String(pick(row, ["REPORT_DATE", "END_DATE", "NOTICE_DATE", "REPORT_YEAR"]) || "").slice(0, 10),
    ...
    revenue_yoy: numberValue(pick(row, ["TOTAL_OPERATE_INCOME_YOY", "OPERATE_INCOME_YOY", "营业总收入同比增长", "营业收入同比增长"])),
    profit_yoy: numberValue(pick(row, ["PARENT_NETPROFIT_YOY", "NETPROFIT_YOY", "归母净利润同比增长", "净利润同比增长"])),
    roe: numberValue(pick(row, ["ROE_WEIGHT", "WEIGHTAVG_ROE", "JQJZCSYL", "净资产收益率"])),
    gross_margin: numberValue(pick(row, ["GROSS_PROFIT_RATIO", "SALE_GROSSPROFITRTO", "XSMLL", "销售毛利率"])),
